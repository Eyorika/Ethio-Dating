...
filtersScene.enter(async (ctx) => {
    ...
    const status = `⚙️ **Current Filters**\n` +
    ...
    await ctx.reply(status, {
        ...
        ...Markup.inlineKeyboard([
            ...
            [Markup.button.callback('📍 Location', 'filter_location')],
            [Markup.button.callback('🗑️ Clear All', 'filter_clear')],
            [Markup.button.callback('🔙 Back to Discovery', 'back_to_discovery')]
        ])
    });
});
...
filtersScene.action('filter_age', async (ctx) => {
    await ctx.editMessageText("📅 **Select Age Range**:", Markup.inlineKeyboard([
        ...
        [Markup.button.callback('🔙 Back', 'filter_home')]
    ]));
});
...
filtersScene.action('filter_location', async (ctx) => {
    await ctx.editMessageText("📍 **Select Location Type**:", Markup.inlineKeyboard([
        [Markup.button.callback('Addis Ababa', 'loc_type_addis')],
        [Markup.button.callback('Outside Addis', 'loc_type_regional')],
        [Markup.button.callback('🔙 Back', 'filter_home')]
    ]));
});

filtersScene.action('loc_type_addis', async (ctx) => {
    // Show sub-cities (paginated or just first few common ones for now, or all if list is small)
    // KIFLE_KETEMAS is likely an array of strings
    const buttons = KIFLE_KETEMAS.map(k => [Markup.button.callback(k, `set_loc_addis_${k}`)]);
    buttons.push([Markup.button.callback('🔙 Back', 'filter_location')]);

    await ctx.editMessageText("📍 **Select Sub-City**:", Markup.inlineKeyboard(buttons));
});
...
filtersScene.action('loc_type_regional', async (ctx) => {
    await ctx.reply("Please type the name of the city you want to filter by (e.g., 'Adama'). 🏙️");
